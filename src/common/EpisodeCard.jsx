...
export default function EpisodeCard({ episode, setIsEpisodePopupOpen, setSelectedEpisode, fetchDetails, isAdmin=false, slug }) {
  ...
  return (
      <div
        className={`group relative flex flex-col md:flex-row items-center gap-6 bg-[#4B4B4B] rounded-2xl shadow-lg overflow-hidden p-6 md:p-8 
      ...
      >
        ...
        <div className="relative w-full md:w-[454px] h-[321px] rounded-2xl overflow-hidden flex-shrink-0">
          ...
          <div className="bg-theme p-5 rounded-full">
            <FaPlay className="text-white text-3xl" size={18}/>
          </div>
          </div>
        </div>
          ...
          <div className="flex items-center text-sm text-[#FFFFFF] gap-4 mb-4">
            <span className="flex items-center gap-1">
              <span className="font-[400] text-[16px] md:text-[18px] px-2 py-1">
                Episode: {episode?.episode?._count?.episodes || 11}
              </span>
              |
            </span>
            <span className="flex items-center gap-1 ">
              <span className="flex items-center md:text-[18px] px-1 py-1 gap-1">
                ...
                <span className="font-[400] text-[16px]">
                  {episode?.author || "Nolan Bator"}
                </span>
              </span> |
            </span>
            <span className="flex items-center gap-1 font-[400] text-[16px]">
              <span className=" px-2 py-1">
                <IoMdTime size={16} className="inline-block" /> {episode?.duration}
                mins
              </span>
            </span>
          </div>
       ...
       {/* Dropdown Menu Trigger - moved to absolute top-right */}
  {isAdmin && (
    <div 
